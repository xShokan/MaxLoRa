...
def frequencyCollision(p1,p2):
    if (abs(p1.freq-p2.freq)<=120 and (p1.bw==500 or p2.freq==500)):
        ...
        if (abs(p1.freq-p2.freq)<=30):
            print("frequency coll 125, node {}, sf {}".format(p1.nodeid, p1.sf))
            return True
        #else:
...
#
class myNode():
    def __init__(self, nodeid, bs, period, packetlen, rssi, sf):
        global df
        self.nodeid = nodeid
        self.period = period
        self.bs = bs
        ...
        while (found == 0 and rounds < 100):
            ...
            if b<a:
                a,b = b,a
            posx = df.loc[nodeid, 'x']
            posy = df.loc[nodeid, 'y']
            if len(nodes) > 0:
        ...
        self.packet = myPacket(self.nodeid, packetlen, self.dist, self.rssi, sf)
        self.sent = 0

        df.loc[nodeid, 'SF'] = self.packet.get_sf()
        # graphics for node
        global graphics
        if (graphics == 1):
            global ax
...
#
class myPacket():
    def __init__(self, nodeid, plen, distance, rssi, sf):
        global experiment
        global Ptx
        global gamma
        global d0
    ...
    while True:
        ...
        if (node in packetsAtBS):
            ...
            if node.packet.rssi < sensitivity:
                ...
                global df_node
                dict0 = {}
                dict0['node'] = node.nodeid
                dict0['sf'] = node.packet.sf
                dict0['time'] = node.packet.addTime
                dict0['rectime'] = node.packet.rectime
                df_node = df_node.append(dict0, ignore_index=True)
        ...
        if node.packet.collided == 1:
            ...
            dict0['sf'] = node.packet.sf
            dict0['time'] = node.packet.addTime
            df_col = df_col.append(dict0, ignore_index=True)
...
def main_func(exp, Num, type0, rate):
    ...
    for i in range(0, nrNodes):
        ...
        # 1000000 = 16 min
        node = myNode(i, bsId, 3600.0 * 1000 / df.loc[i, rate], 20, df.loc[i, 'RSSI'], int(df.loc[i, 'SF']))
        nodes.append(node)
    ...
    df_col = pd.DataFrame(columns=['node', 'sf', 'time'])
    df_node = pd.DataFrame(columns=['index', 'node', 'sf', 'time', 'rectime'])
    count = 0
    # start simulation
    ...
    fname = "exp_lambda" + str(experiment) + '_' + str(len(nodes)) + ".dat"
    print(fname)
    if os.path.isfile(fname):
        res = "\n" + str(nrNodes) + " " + str(nrCollisions) + " " + str(sent) + " " + str(energy) + " " + str(
            1 - nrCollisions / sent)
    else:
        res = "#nrNodes nrCollisions nrTransmissions OverallEnergy PDR\n" + str(nrNodes) + " " + str(
            nrCollisions) + " " + str(sent) + " " + str(energy) + " " + str(1 - nrCollisions / sent)
